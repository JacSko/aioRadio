...
{
   return g_rtosMock->xQueueSendFromISR(xQueue, pvItemToQueue, pxHigherPriorityTaskWoken);
}
BaseType_t xQueueSend(QueueHandle_t xQueue,
                      const void * const pvItemToQueue,
                      BaseType_t * const pxHigherPriorityTaskWoken)
{
   return g_rtosMock->xQueueSend(xQueue, pvItemToQueue, pxHigherPriorityTaskWoken);
}
BaseType_t xQueueReceive(QueueHandle_t xQueue,
                         void * pvItemToQueue,
                         uint32_t xTicksToWait)
{
   return g_rtosMock->xQueueReceive(xQueue, pvItemToQueue, xTicksToWait);
}
